...
namespace Selkie.Geometry.Tests.Shapes.NUnit
{
    ...
    internal sealed class PointTests
    {
        ...
        {
            var centre = new Point(10.0, 10.0);
            var one = new Point(10.0, 20.0);
            var two = new Point(20.0, 0.0);

            Angle expected = Angle.For135Degrees;
            Angle actual = centre.AngleBetweenPoints(one, two);
        ...
        {
            var other = new Point(3.0, 8.0);

            const double expected = 4.0;
            double actual = m_Point.DistanceTo(other);

            NUnitHelper.AssertIsEquivalent(expected, actual, "Distance");
        }
        ...
        public void EqualsReturnsFalseForSameValueWithDeltaTest()
        {
            ...
            var point2 = new Point(1.0 + SelkieConstants.EpsilonPointXy, 2.0 + SelkieConstants.EpsilonPointXy);
        ...
        {
            var point1 = new Point(1.0, 2.0);
            var point2 = new Point(1.0 + SelkieConstants.EpsilonPointXy/2.0, 2.0 + SelkieConstants.EpsilonPointXy/2.0);
        ...
        public void Move180DegreesTest()
        {
            ...
            Point moved = m_Point.Move(distance, radians);

            NUnitHelper.AssertIsEquivalent(m_Point.X, moved.X, "X");
            NUnitHelper.AssertIsEquivalent(-6.0, moved.Y, "Y");
        }
        ...
        public void Move225DegreesTest()
        {
            ...
            NUnitHelper.AssertIsEquivalent(-4.07, moved.X, "X");
            NUnitHelper.AssertIsEquivalent(-3.07, moved.Y, "Y");
        }
        ...
        public void Move45DegreesTest()
        {
            ...
            NUnitHelper.AssertIsEquivalent(10.07, moved.X, "X");
            NUnitHelper.AssertIsEquivalent(11.07, moved.Y, "Y");
        }
        ...
        public void Move90DegreesTest()
        {
            ...
            NUnitHelper.AssertIsEquivalent(m_Point.X + 10.0, moved.X, "X");
            NUnitHelper.AssertIsEquivalent(m_Point.Y, moved.Y, "Y");
        }
        ...
        public void RelativeToForSamePointTest()
        {
            ...
            Assert.AreEqual(0.0, actual.X, "X");
            Assert.AreEqual(0.0, actual.Y, "Y");
        }
        ...
        {
            var point = new Point(1.0, 2.0);
            var other = new Point(-2.0, 4.0);

            Point actual = point.RelativeTo(other);

            Assert.AreEqual(3.0, actual.X, "X");
            Assert.AreEqual(-2.0, actual.Y, "Y");
        }

        [Test]
        public void RelativeToQuadrantFourCaseTwoTest()
        {
            var point = new Point(-5.0, 10.0);
            var other = new Point(-2.0, 4.0);

            Point actual = point.RelativeTo(other);

            Assert.AreEqual(-3.0, actual.X, "X");
            Assert.AreEqual(6.0, actual.Y, "Y");
        }
        ...
        public void RelativeToQuadrantOneCaseOneTest()
        {
            ...
            Assert.AreEqual(-1.0, actual.X, "X");
            Assert.AreEqual(-2.0, actual.Y, "Y");
        }
        ...
        public void UnknownInstanceTest()
        {
            ...
            Assert.True(point.IsUnknown, "IsUnknown");
            Assert.AreEqual(double.NaN, point.X, "X");
            Assert.AreEqual(double.NaN, point.Y, "Y");
        }
        ...
        public void XTest()
        {
            Assert.AreEqual(3.0, m_Point.X);
        }

        [Test]
        public void YTest()
        {
            Assert.AreEqual(4.0, m_Point.Y);
        }
    }
}