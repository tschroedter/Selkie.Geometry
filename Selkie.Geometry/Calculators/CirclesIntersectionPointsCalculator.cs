    ...
    public class CirclesIntersectionPointsCalculator : ICirclesIntersectionPointsCalculator
    {
        public CirclesIntersectionPointsCalculator([NotNull] ICirclePair circlePair)
        {
            ...
            if ( IsSameCircles(circlePair,
                               ...
                               r0MinusR1Abs) )
            {
                IsCirclesAreSame = true;
                HasIntersectionPoints = false;
            }
            else if ( r0PlusR1 >= d &&
                      d >= r0MinusR1Abs )
            {
                ...
                IsCirclesTouchAtSinglePoint = DetermineNumberOfIntersectionPoints(points) == 1;

                if ( IsCirclesTouchAtSinglePoint )
                {
                    ...
                    IntersectionPointOne = point;
                    IntersectionPointTwo = point;
                }
                else
                {
                    IntersectionPointOne = points.Item1;
                    IntersectionPointTwo = points.Item2;
                }
        ...
        public bool HasIntersectionPoints { get; }

        public bool IsCirclesAreSame { get; }

        public bool IsCirclesTouchAtSinglePoint { get; }

        public Point IntersectionPointOne { get; } = Point.Unknown;

        public Point IntersectionPointTwo { get; } = Point.Unknown;
